...
import { layerCapFor } from "./lib/network";
...
export function autoBackward(
  ...
  for (const node of addNodes || []) {
    ...
    // 容量硬约束：网络配置 layerCaps（无配置默认40）。used>=cap 拒绝新增，迫使 LLM 走 merge/node_updates。
    const capN = layerCapFor(net.hyperparams, targetLayer);
    if (existingNodes.length >= capN) {
      nodesUpdatedSkipped++;
      nodeSkipReasons.push(`L${targetLayer}:over_cap(${existingNodes.length}/${capN})`);
      onLog(`Textron backward: skipped add_node L${targetLayer} (over cap ${existingNodes.length}/${capN}) — merge or update existing nodes first`);
      continue;
    }
